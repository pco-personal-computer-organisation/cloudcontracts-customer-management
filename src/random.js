...
const randomName = () => { // taken from https://github.com/moby/moby/blob/master/pkg/namesgenerator/names-generator.go
  const left = [ // 93 items
    'admiring',
    'adoring',
    'affectionate',
...
const getRandomNameWithMaxLength = (length) => {
  const name = randomName();

  return name.length <= length - 2 ? `${name}${Math.floor(Math.random() * 100)}` : getRandomNameWithMaxLength(length);
};

module.exports = {
  randomString,
  getRandomNameWithMaxLength,
}
